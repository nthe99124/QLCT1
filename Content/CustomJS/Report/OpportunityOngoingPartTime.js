$(document).ready(function () {
    ...
    $('#OpportunityOngoingPartTime').dataTable({
        paging: false,
        "searching": false,
        "bInfo": false,
        "fixedHeader": {
    ...
    setTimeout(function () {
        ...
        if (lstDiv != null) {
            ...
            $('#bidTreeOppOngoingPartTime').jstree("deselect_all");
            $("#bidTreeOppOngoingPartTime").jstree(true).select_node(divArr);
        }
    }, 300);
...
$('#btnSubmitSearch').click(function () {
    var lstDiv = ConvertListIdString($('#bidTreeOppOngoingPartTime').jstree('get_selected'));
    $("#lstIdStr").val(lstDiv);
...
var UITree = function () {
    var contextualMenu = function () {
        //render cây division
        $("#bidTreeOppOngoingPartTime").jstree({
            "core": {
                "themes": {
                    "responsive": true
                },
        ...
        $("#bidTreeOppOngoingPartTime").bind("changed.jstree",
            function (e, data) {
            });
    }
    return {
...
//$("#bidTreeOppOngoingPartTime").on("select_node.jstree", function (node, selected) {
//    $("#bidTreeOppOngoingPartTime").jstree('open_all');
//    var siblingNodes = $("#bidTreeOppOngoingPartTime").jstree(true).get_children_dom(selected.node.id);
//    var allChecked = true;
...
//                $("#bidTreeOppOngoingPartTime").jstree(true).select_node(this);
//            });
//            $("#bidTreeOppOngoingPartTime").jstree(true).select_node(selected.node.id);
//        }
//    }
//});

//$("#bidTreeOppOngoingPartTime").on("deselect_node.jstree", function (node, deselected) {
//    $("#bidTreeOppOngoingPartTime").jstree('open_all');
//    var siblingNodes = $("#bidTreeOppOngoingPartTime").jstree(true).get_children_dom(deselected.node.id);
...
//        if (allChecked) {
//            $(siblingNodes).each(function () {
//                $("#bidTreeOppOngoingPartTime").jstree(true).deselect_node(this);
//            });
//            $("#bidTreeOppOngoingPartTime").jstree(true).deselect_node(deselected.node.id);
//        }
//    }
//});
...
$("#bidTreeOppOngoingPartTime").on("select_node.jstree", function (node, selected) {
    var siblingNodes = $("#bidTreeOppOngoingPartTime").jstree(true).get_node(selected.node.id).children_d;
    //chọn hết các con 
    if (siblingNodes.length > 0) {
        $.each(siblingNodes, function (id, vl) {
            $("#bidTreeOppOngoingPartTime").jstree(true).select_node(vl);
        });
    }
    if ($("#boolClearTree").val() == "True") {
        var listChildren = [];
        var $tree = $('#bidTreeOppOngoingPartTime');
        ...
        $.each(listChildren, function (index, value) {
            var obj = $(value)[0];
            setTimeout(function () { $('#bidTreeOppOngoingPartTime').jstree(true).deselect_node(obj.id); }, 5);
        });
    }
});

$("#bidTreeOppOngoingPartTime").on("deselect_node.jstree", function (node, deselected) {
    var siblingNodes = $("#bidTreeOppOngoingPartTime").jstree(true).get_node(deselected.node.id).children_d;

    //bỏ chọn hết các con 
    if (siblingNodes.length > 0) {
        $.each(siblingNodes, function (id, vl) {
            $("#bidTreeOppOngoingPartTime").jstree(true).deselect_node(vl);
        });
    }
    if ($("#boolClearTree").val() == "True") {
        var listChildren = [];
